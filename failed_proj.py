...
chrome.get("https://www.flyingv.cc/projects?filter=all&sort=end&category=product")
...
for project in projectList:
    ...
    percent = getValidStr(project.find('span', {'class': 'hidden-md goalpercent'}))
    # print(title, creator)
    ...
    ### start time & end time ###
    timeRaw = projectSoup.find(class_='col-sm-4 sidebar numberSidebar').blockquote
    if(timeRaw == None):
...
test.to_excel(outputFileName + '_failed.xlsx', encoding='utf-8')
